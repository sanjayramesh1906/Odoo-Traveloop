...
// Using standard browser fetch. Alternatively, could use axios if it's the team standard.
...
export default function BudgetScreen({ tripId = '1' }) {
  ...
  const handleDeleteExpense = async (itemId) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/trips/${tripId}/budget/${itemId}`, {
        method: 'DELETE',
      });
  ...
  if (loading && !budgetData) return <div className="budget-container">Loading...</div>;
  if (error) return <div className="budget-container">Error: {error}</div>;
  if (!budgetData) return null;

  return (
    <div className="budget-container">
      <div className="budget-header">
        <h1>Trip Budget & Costs</h1>
        <p>Track your expenses and stay on top of your spending</p>
      </div>
                ...
                {budgetData.items.map(item => (
                  <div key={item.id} className="expense-item">
                    <div className="expense-info">
                      ...
                      {item.date && <span className="expense-date">{item.date}</span>}
                    </div>
